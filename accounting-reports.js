...
function generateIncomeStatementFormA(opts = {}) {
  ...
  const l17 = b('7010') + b('7011') + b('7012') + b('7013'); // Цалин хөлс, шагнал (Хоол/Унаа/Утасны мөнгийг хуулиар "цалинтай адилтгах орлого" гэж vзнэ)
...
// бvгд <table> биш, .summary-row div-vvдээр (_acctRow) баригдсан тул
// нэгдсэн, ерөнхий export функцээр (DOM-оос шүүрж) Excel vvсгэнэ.
function exportSummaryRowsToXlsx(containerId, filename) {
  const container = document.getElementById(containerId);
  if(!container) return;
  const rows = container.querySelectorAll('.summary-row');
  if(!rows.length) { toast('Экспортлох мэдээлэл олдсонгvй', 'error'); return; }
  try {
    const aoa = [];
    rows.forEach(r => {
  ...
  el.innerHTML = `
    <div class="flex-between mb-16">
      ...
      <div class="flex gap-8" style="align-items:center">
        ...
        <button class="btn btn-outline btn-sm" onclick="printCurrentPage()" title="Хэвлэх">Хэвлэх</button>
        <button class="btn btn-outline btn-sm" onclick="exportSummaryRowsToXlsx('acct-balance-sheet','Тэнцэл.xlsx')" title="Excel экспорт">Экспорт</button>
      </div>
    </div>
  ...
  el.innerHTML = `
    <div class="flex-between mb-16">
      ...
      <div class="flex gap-8">
        <button class="btn btn-outline btn-sm" onclick="printCurrentPage()" title="Хэвлэх">Хэвлэх</button>
        <button class="btn btn-outline btn-sm" onclick="exportSummaryRowsToXlsx('acct-income-statement','Орлогын_тайлан.xlsx')" title="Excel экспорт">Экспорт</button>
      </div>
    </div>
    <div class="card mb-16" style="padding:18px">
  ...
  el.innerHTML = `
    <div class="flex-between mb-16">
      ...
      <div class="flex gap-8" style="align-items:center">
        ...
        <button class="btn btn-outline btn-sm" onclick="printCurrentPage()" title="Хэвлэх">Хэвлэх</button>
        <button class="btn btn-outline btn-sm" onclick="exportSummaryRowsToXlsx('acct-cash-flow','Мөнгөн_гүйлгээ.xlsx')" title="Excel экспорт">Экспорт</button>
      </div>
    </div>
    <div class="card mb-16" style="padding:18px">
...
function openInvoiceDetailModal(partyKey) {
  ...
  document.getElementById('invoice-detail-body').innerHTML = `
    ...
    <div class="summary-row" style="font-weight:700"><span class="summary-key">Нийт дvн</span><span class="summary-val">${fmtMoney(row.amount)}</span></div>`;
...
async function renderSalaryComponentsSettingsPage() {
  ...
  if (error) { el.innerHTML = '<div class="empty-state">Ачаалахад алдаа гарлаа</div>'; return; }
  if (!data.length) { el.innerHTML = '<div class="empty-state">Нэмэгдэл бүртгэгдээгvй байна</div>'; return; }
    ...
    <div class="card mb-16" style="padding:18px">
      <div class="flex-between mb-16">
        <div>
          ...
          <div style="font-size:11px;color:var(--text-muted);margin-top:2px">Код: ${esc(c.code)} · Дт данс: ${esc(c.expense_account) || '—'}</div>
        </div>
        <div style="display:flex;gap:8px;align-items:center">
          <span class="tag ${c.enabled ? 'tag-success' : ''}" style="cursor:pointer" onclick="toggleSalaryComponentEnabled('${esc(c.code)}', ${!c.enabled})">${c.enabled ? 'Идэвхтэй' : 'Идэвхгvй'}</span>
      ...
      <div class="summary-row"><span class="summary-key" style="padding-left:16px">Дүн (сард, бүх ажилтанд ижил)</span><span class="summary-val">${fmtMoney(c.amount)}</span></div>
      <div class="summary-row"><span class="summary-key" style="padding-left:16px">ХХОАТ-д тооцох</span><span class="summary-val">${c.hhoat_taxable ? 'Тийм' : 'Vгvй'}</span></div>
      <div class="summary-row"><span class="summary-key" style="padding-left:16px">НДШ-д тооцох</span><span class="summary-val">${c.ndsh_taxable ? 'Тийм' : 'Vгvй'}</span></div>
...
async function saveSalaryComponent() {
  ...
  if (!code || !name) { toast('Код болон нэрийг бөглөнө vv', 'error'); return; }
  const row = {
    code, name,
...
async function renderTaxTab() {
  ...
  el.innerHTML = `
    <div class="flex-between mb-16" style="align-items:flex-start">
      ...
      <div class="flex gap-8">
        <button class="btn btn-outline btn-sm" onclick="printCurrentPage()" title="Хэвлэх">Хэвлэх</button>
        <button class="btn btn-outline btn-sm" onclick="exportSummaryRowsToXlsx('acct-tax','Татварын_тайлан.xlsx')" title="Excel экспорт">Экспорт</button>
      </div>
    </div>
