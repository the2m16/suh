...
async function renderSalaryComponentsSettingsPage() {
  ...
  if (error) { el.innerHTML = '<div class="empty-state">Ачаалахад алдаа гарлаа</div>'; return; }
  if (!data.length) { el.innerHTML = '<div class="empty-state">Цалингийн нэмэгдэл бvртгэгдээгvй байна</div>'; return; }
    ...
    <div class="card mb-16" style="padding:18px">
      <div class="flex-between mb-16">
        <div>
          ...
          <div style="font-size:11px;color:var(--text-muted);margin-top:2px">Код: ${esc(c.code)}</div>
        </div>
        <div style="display:flex;gap:8px;align-items:center">
