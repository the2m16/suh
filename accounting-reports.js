...
const CHART_OF_ACCOUNTS = [
  ...
  { code: '7013', name: 'Утасны мөнгөний зардал',                category: 'expense', group: 'Ерөнхий удирдлагын зардал', official_code: '70',
    note: 'Хуульд шууд дурдаагvй, "адилтгах орлого"-д өргөн тайлбарласан' },
...
// ============================================================
// TAB: НЭХЭМЖЛЭХ ИЛГЭЭХ — "Гvйлгээний бvртгэл" хуудаснаас энд НvvЛГЭСЭН.
// Идэвхтэй (илгээгээгvй) сар бол ЛАЙВ төсөөлөл (хасах боломжтой),
...
function excludeFromInvoice(partyKey) {
  if (!confirm('Энэ хvлээн авагчийг ЭНЭ САРЫН нэхэмжлэлээс хасах уу?')) return;
...
async function saveTaxType() {
  ...
  // 'progressive' төрөлд rate талбарууд хамаагvй — tax_brackets хvснэгэлээр удирдагдана,
  // энд заавал NULL vлдэнэ (дээрх мөрvvдэд аль хэдийн тооцогдсон)
...
async function renderSalaryComponentsSettingsPage() {
  ...
  if (error) { el.innerHTML = '<div class="empty-state">Ачаалахад алдаа гарлаа</div>'; return; }
  if (!data.length) { el.innerHTML = '<div class="empty-state">Нэмэгдэл бvртгэгдээгvй байна</div>'; return; }
  ...
  el.innerHTML = data.map(c => `
    <div class="card mb-16" style="padding:18px">
      ...
      <div class="summary-row"><span class="summary-key" style="padding-left:16px">Дvн (сард, бvх ажилтанд ижил)</span><span class="summary-val">${fmtMoney(c.amount)}</span></div>
...
function openEditSalaryComponent(c) {
  ...
  document.getElementById('salary-component-code').disabled = true; // код — PK тул засварлахгvй
