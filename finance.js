...
function switchFinTab(name, el) {
  ['fin-income','fin-expenses','fin-accumulation'].forEach(id=>{const e=document.getElementById(id);if(e)e.style.display='none';});
...
function renderAccumulation() {
  // Dropdown-д байгаа жилүүдийг тогтоох ("Бүх он" анхны утга)
  ...
  const yearRaw = sel ? sel.value : '';
  const selectedYear = yearRaw ? +yearRaw : null;
  const titleEl = document.getElementById('accumulation-card-title');
  if(titleEl) titleEl.textContent = selectedYear ? (selectedYear + ' оны хуримтлал') : 'Бүх оны хуримтлал';

  const tb=document.getElementById('accumulation-table-body');

  if(selectedYear) {
    // Тодорхой жил сонгосон бол — 12 сарын задаргаа
    ...
    const rows=months.map(m=>{
      ...
      return `<tr style="${!hasTx?'opacity:.3':''}">
        ...
        <td class="text-success dt-mono">${inc?fmtMoney(inc):'—'}</td>
        <td class="text-danger dt-mono">${exp?fmtMoney(exp):'—'}</td>
        <td class="text-warning dt-mono">${fund?fmtMoney(fund):'—'}</td>
        <td class="${diff>=0?'text-success':'text-danger'} dt-mono">${hasTx?fmtMoney(diff):'—'}</td>
        <td class="text-accent dt-mono">${hasTx?fmtMoney(cumulative):'—'}</td>
      </tr>`;
    });
    if(tb) tb.innerHTML=rows.join('');
  } else {
    // "Бүх он" сонгосон бол — жил бүрээр нэгтгэсэн задаргаа
    const years2 = [...new Set(transactions.filter(Boolean).map(t=>t.year).filter(Boolean))].sort((a,b)=>a-b);
    let cumulative=0;
    const rows=years2.map(y=>{
      const inc=transactions.filter(t=>t&&t.type==='income'&&t.year===y).reduce((s,t)=>s+t.amount,0);
      const expAll=transactions.filter(t=>t&&t.type==='expense'&&t.year===y).reduce((s,t)=>s+t.amount,0);
      const fund=transactions.filter(t=>t&&t.type==='expense'&&t.year===y&&t.subcat==='Хуримтлалын сан').reduce((s,t)=>s+t.amount,0);
      const exp=expAll-fund;
      const diff=inc-expAll; const hasTx=inc>0||expAll>0;
      if(hasTx) cumulative+=diff;
      return `<tr style="${!hasTx?'opacity:.3':''}">
  ...
  // Зүүн талын инфо — сонгосон жилээр, эсвэл "Бүх он" үед бүх цагийн нийлбэрээр
  const yearMatch = t => !selectedYear || t.year===selectedYear;
  const totInc=transactions.filter(t=>t&&t.type==='income'&&yearMatch(t)).reduce((s,t)=>s+t.amount,0);
  const totExpAll=transactions.filter(t=>t&&t.type==='expense'&&yearMatch(t)).reduce((s,t)=>s+t.amount,0);
  const totFund=transactions.filter(t=>t&&t.type==='expense'&&yearMatch(t)&&t.subcat==='Хуримтлалын сан').reduce((s,t)=>s+t.amount,0);
