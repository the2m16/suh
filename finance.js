...
function _fmtTxTime(createdAt) {
  if(!createdAt) return { hour:'—', minute:'—' };
...
function renderIncomeTable() {
  ...
  body.innerHTML=list.map(t=>{
    const tm=_fmtTxTime(t.createdAt);
    // "Сvvлийн гvйлгээ" картын адил загвар: аж ахуйн нэгж бол нэрийг нь,
    ...
    return `<tr>
      ...
      <td class="dt-muted dt-mono">${tm.hour}</td>
      <td class="dt-muted dt-mono">${tm.minute}</td>
  ...
  }).join('')||'<tr><td colspan="8" style="text-align:center;padding:20px;color:var(--text-muted)">Орлого байхгүй</td></tr>';
}
function renderExpenseTable() {
  ...
  }).join('')||'<tr><td colspan="8" style="text-align:center;padding:20px;color:var(--text-muted)">Зарлага байхгүй</td></tr>';
}
...
// Хvлээлттэй/Хугацаа хэтэрсэн/Эрсдэлтэй tab бvрийн тоог (сууц өмчлөгч+
// аж ахуйн нэгж хамт) нэг дороос тооцоолно — tab badge, sidebar badge хоёул
// ЯГ ЭНЭ НЭГ функцийг ашиглана (тоо зөрvvлэхгvйн тулд).
