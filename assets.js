...
// "Бүх хариуцагч"/"Бүх байршил" шүүлтvvр dropdown-уудыг угсарна (одоо байгаа
...
function renderAssetCategorySettingsList() {
  ...
  if(!el) return;
  if(!ASSET_CATEGORIES.length) { el.innerHTML = '<div class="empty-state">Ангилал бүртгэгдээгvй байна</div>'; return; }
...
async function deleteAssetCategory(code) {
  ...
  const msg = typeCount>0
    ? `Энэ ангилалыг устгах уу? Дотор нь бvртгэлтэй ${typeCount} төрөл ч мөн хамт устгагдана.`
...
function renderAssetTypeSettingsList() {
  ...
  if(!list.length) { el.innerHTML = '<div class="empty-state">Энэ ангилалд төрөл бүртгэгдээгvй байна</div>'; return; }
...
function renderAssetLocationSettingsList() {
  ...
  if(!el) return;
  if(!ASSET_LOCATIONS.length) { el.innerHTML = '<div class="empty-state">Байршил бүртгэгдээгvй байна</div>'; return; }
