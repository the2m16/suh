...
function populateAssetFilterSelects() {
  ['list','depreciation'].forEach(tab=>{
    ...
    if(respSel) {
      ...
      respSel.innerHTML = '<option value="">Бүх хариуцагч</option>' + employees.map(e=>`<option value="${esc(e.fullName)}">${esc(e.fullName)}</option>`).join('');
...
function _assetActionIcons(a, canEditA, canDelA) {
  return _rowActionIcons(a.id, canEditA, canDelA, 'editAsset', 'deleteAsset', canCopy('assets'), 'copyAsset');
}
...
function populateAssetResponsibleSelect(keepValue) {
  ...
  if(!sel) return;
  sel.innerHTML = '<option value="">— Хариуцагч сонгох —</option>' + employees.map(e=>`<option value="${esc(e.fullName)}">${esc(e.fullName)}</option>`).join('');
  if(keepValue) sel.value = keepValue;
}
...
// Хуулбарлах — editAsset()-ийн бүх талбарыг pre-fill хийгээд, editingAssetId-г null
// болгосноор saveAsset() UPDATE биш шинэ INSERT хийнэ. Кодыг (шинэ хөрөнгө өөр код
// авах ёстой тул) хоослож, хэрэглэгчээр шинээр оруулуулна.
function copyAsset(id) {
  if(!canCopy('assets')) { toast('Танд энэ үйлдлийг хийх эрх байхгүй байна','error'); return; }
  editAsset(id);
  editingAssetId = null;
  document.getElementById('modal-asset-title').textContent = 'Хөрөнгө хуулбарлах (шинэ код өгнө үү)';
...
async function saveAssetLocation() {
  ...
  const { error } = await sb.from('asset_locations').insert({ label });
