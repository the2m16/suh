...
async function syncMonthlyDepreciationExpenses() {
  ...
  for(const {a, amount} of toInsert) {
    ...
    // Нягтлан бодох бүртгэлийн журнал бичилт (нэмэлт) — мөнгөн бус зардал тул cash дансад хүрэхгүй
    if (typeof accountingRecordDepreciation === 'function') {
      accountingRecordDepreciation(a.name, amount, row.date)
...
async function saveAsset() {
  ...
  // Нягтлан бодох бүртгэлийн журнал бичилт (нэмэлт — зөвхөн ШИНЭ хөрөнгөнд, засварт биш)
  if (isNewAsset && typeof accountingRecordAssetPurchase === 'function') {
    accountingRecordAssetPurchase(name, data.cost, data.purchaseDate || todayStr())
      .then(res => { if (!res.success) console.warn('Journal entry үүсгэхэд алдаа:', res.error); })
      .catch(e => console.warn('Journal entry үүсгэхэд алдаа:', e));
  }
...
async function confirmDisposeAsset() {
  ...
  if (typeof accountingRecordAssetDisposal === 'function') {
    const {accumulated} = computeDepreciation({...a, status:'disposed', disposalDate: updated.disposalDate});
    accountingRecordAssetDisposal(a.name, a.cost, accumulated, updated.disposalValue, updated.disposalDate || todayStr())
      .then(res => { if (!res.success) console.warn('Journal entry үүсгэхэд алдаа:', res.error); })
      .catch(e => console.warn('Journal entry үүсгэхэд алдаа:', e));
  }
