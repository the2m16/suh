...
async function renderPollsPage() {
  ...
  const canManage = ['admin','gz'].includes(currentProfile?.role);
  ...
  wrap.innerHTML = pollsList.map(p => {
    ...
    const typeLabel = p.poll_type === 'issue' ? 'Санал асуулга' : 'Ээлжит сонгууль';
...
function openCreatePoll() {
  ...
  document.querySelector('input[name="poll-type"][value="issue"]').checked = true;
...
function setPollType(type) {
  ...
  const issueBtn = document.getElementById('poll-type-btn-issue');
  const electionBtn = document.getElementById('poll-type-btn-election');
  if(issueBtn) {
    issueBtn.style.border = type==='issue' ? '2px solid var(--accent)' : '1px solid var(--border)';
    issueBtn.style.background = type==='issue' ? 'var(--accent-glow)' : 'transparent';
  }
  if(electionBtn) {
    electionBtn.style.border = type==='election' ? '2px solid var(--accent)' : '1px solid var(--border)';
    electionBtn.style.background = type==='election' ? 'var(--accent-glow)' : 'transparent';
  }
  document.getElementById('poll-issues-wrap').style.display = type==='issue' ? 'block' : 'none';
...
function renderIssuesList() {
  ...
  if(!currentPollQuestions.length) {
    wrap.innerHTML = '<div style="font-size:12px;color:var(--text-muted);padding:8px 0">Асуудал нэмээгүй байна</div>';
    return;
  }
  wrap.innerHTML = currentPollQuestions.map((q,i)=>`
    <div style="display:flex;gap:8px;margin-bottom:8px;align-items:center">
      ...
      <input type="text" value="${q.text}" placeholder="ж: Зогсоолын хураамж нэмэгдүүлэх"
...
function removeCandidate(col, id) {
  setCandidatesArr(col, getCandidatesArr(col).filter(c=>c.id!==id));
  renderCandidatesList(col);
}
...
function updateCandidateResident(col, id, residentId) {
  ...
  c.name = r ? `${r.lastname||''} ${r.firstname||''}`.trim() : '';
}
...
async function savePoll() {
  ...
  if(pollType === 'issue' && currentPollQuestions.filter(q=>q.text.trim()).length === 0) {
    toast('Дор хаяж нэг асуудал нэмнэ үү', 'error'); return;
  }
  ...
  if(pollType === 'issue') {
    for(let i=0; i<currentPollQuestions.length; i++) {
      ...
      if(qErr) { toast('Асуудал хадгалахад алдаа: '+qErr.message,'error'); continue; }
...
async function openPollDetail(pollId) {
  ...
  let html = `<div class="card" style="padding:16px 18px;margin-bottom:16px">
    <div style="font-size:13px;color:var(--text-dim)">${poll.description || ''}</div>
    <div style="font-size:12px;color:var(--text-muted);margin-top:8px">
      Төлөв: <strong>${poll.status === 'draft'?'Ноорог':poll.status==='active'?'Идэвхтэй':'Хаагдсан'}</strong>
  ...
  if(canManage && poll.status === 'draft') {
    html += `<div style="margin-bottom:16px;display:flex;gap:8px">
      ...
      <button class="btn btn-primary btn-sm" onclick="activatePoll(${pollId})">Санал хураалт эхлүүлэх</button>
    </div>`;
  }
  ...
  for(const q of (questions||[])) {
    const {data: options} = await sb.from('poll_options').select('*').eq('question_id', q.id).order('order_num');
    const {data: myVotes} = await sb.from('votes').select('option_id').eq('question_id', q.id).eq('resident_id', currentProfile?.apt || -1);
    const myVoteIds = (myVotes||[]).map(v=>v.option_id);
    const hasVoted = myVoteIds.length > 0;

    // Санал өгөх UI (active + owner + санал өгөөгүй)
    if(poll.status === 'active' && isOwner && !hasVoted) {
      const inputType = q.max_selections > 1 ? 'checkbox' : 'radio';
      html += `<div class="card" style="padding:16px 18px;margin-bottom:12px">
        <div style="font-weight:600;font-size:13px;margin-bottom:10px">${q.question_text}</div>`;
      html += (options||[]).map(o=>`
        <label style="display:flex;align-items:center;gap:8px;padding:8px 0;cursor:pointer">
          <input type="${inputType}" name="q${q.id}" value="${o.id}">
          <span style="font-size:13px">${o.option_text}</span>
        </label>`).join('');
      html += `<button class="btn btn-primary btn-sm" style="margin-top:10px" onclick="submitVote(${q.id}, ${poll.id}, '${poll.poll_type}')">Санал өгөх</button>
      </div>`;
      continue;
    }

    // Үр дүн — residents table-тэй адил дизайн
    let {data: allVotes} = (poll.show_results_live || poll.status === 'closed' || canManage)
      ? await sb.from('votes').select('option_id').eq('question_id', q.id)
      : {data: []};
    const counts = {};
    (allVotes||[]).forEach(v => counts[v.option_id] = (counts[v.option_id]||0)+1);
    const totalVotes = (allVotes||[]).length || 1;

    html += `<div style="overflow:hidden;margin-bottom:16px">
      <table class="poll-result-table" style="width:100%;border-collapse:collapse">
        <thead>
          <tr style="background:#0B1A2E">
            <th style="font-size:9px;font-weight:700;color:#475569;letter-spacing:.07em;padding:7px 10px;border-bottom:1px solid var(--border);text-align:left">${q.question_text}</th>
            <th style="font-size:9px;font-weight:700;color:#475569;letter-spacing:.07em;padding:7px 10px;border-bottom:1px solid var(--border);text-align:right;width:80px">САНАЛ</th>
            <th style="font-size:9px;font-weight:700;color:#475569;letter-spacing:.07em;padding:7px 10px;border-bottom:1px solid var(--border);text-align:right;width:60px">%</th>
            <th style="font-size:9px;font-weight:700;color:#475569;letter-spacing:.07em;padding:7px 10px;border-bottom:1px solid var(--border);width:180px"></th>
          </tr>
        </thead>
        <tbody>`;
    ...
    (options||[]).forEach(o => {
      ...
      html += `<tr style="${rowBg}">
        <td style="padding:7px 10px;font-size:12px;border-bottom:1px solid rgba(30,58,95,0.25);vertical-align:middle">
          ${isMyVote ? '<span style="color:var(--accent);font-size:10px;margin-right:4px">✓</span>' : ''}${o.option_text}
        </td>
        <td style="padding:7px 10px;font-size:12px;border-bottom:1px solid rgba(30,58,95,0.25);text-align:right;font-variant-numeric:tabular-nums">${cnt}</td>
        <td style="padding:7px 10px;font-size:12px;border-bottom:1px solid rgba(30,58,95,0.25);text-align:right;color:var(--text-muted);font-variant-numeric:tabular-nums">${pct}%</td>
        <td style="padding:7px 10px;border-bottom:1px solid rgba(30,58,95,0.25);vertical-align:middle">
...
async function submitVote(questionId, pollId, pollType) {
  const selected = [...document.querySelectorAll(`input[name="q${questionId}"]:checked`)].map(el=>+el.value);
  if(!selected.length) { toast('Сонголт хийнэ үү', 'error'); return; }

  const residentApt = currentProfile?.apt;
  if(!residentApt) { toast('Таны профайлд тоот холбогдоогүй байна', 'error'); return; }

  const rows = selected.map(optionId => ({
    poll_id: pollId, question_id: questionId, option_id: optionId,
    resident_id: residentApt, apt: residentApt, poll_type: pollType
  }));
  ...
  toast('Санал амжилттай бүртгэгдлээ ✓', 'success');
