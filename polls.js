  ...
  for(const q of (questions||[])) {
    ...
    if(poll.status === 'active' && isOwner && !hasVoted) {
      ...
      html += `<div class="card" style="padding:16px 18px;margin-bottom:12px">
        ...
        <div style="display:flex;flex-direction:column;gap:8px" id="vote-options-${q.id}">`;
      html += (options||[]).map(o=>`
      ...
      html += `</div>
        <button class="btn btn-primary btn-sm" style="margin-top:12px" onclick="submitVote(${q.id}, ${poll.id}, '${poll.poll_type}')">Санал өгөх</button>
      </div>`;
      continue;
    }

    // Үр дүн — aggregate RPC ашиглана (хэн ямар сонголт хийснийг задруулахгvй, зөвхөн нийт тоог)
    let counts = {}, totalVotes = 1;
    if(poll.show_results_live || poll.status === 'closed' || canManage) {
      ...
      totalVotes = (results||[]).reduce((s,r)=>s+r.vote_count,0) || 1;
    }
...
async function submitVote(questionId, pollId, pollType) {
  const selected = [...document.querySelectorAll(`.vote-option-btn[data-question="${questionId}"].selected`)].map(el=>+el.dataset.option);
