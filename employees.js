...
function buildPayrollLinesGeneric(party, result) {
  const lines = [];
  lines.push({ account: '7010', debit: result.grossSalary, credit: 0, party });
...
// ------------------------------------------------------------
// ЦАЛИНГИЙН НЭМЭГДЭЛ (Хоол/Унаа/Утас) — глобаль каталог + ажилтны хэрэглээ
// ------------------------------------------------------------
...
// хэлбэрээр (component config + дvн нэгтгэсэн) буцаана.
async function loadEmployeeSalaryComponents(employeeId) {
  ...
  return (data || []).map(row => {
    ...
    return { code: row.component_code, name: cat.name || row.component_code, amount: +row.amount,
...
async function renderEmployeeSalaryComponentsUI(existingComponents = []) {
  ...
  container.innerHTML = catalog.map(c => {
    ...
    <div style="display:flex;align-items:center;gap:10px" data-comp-code="${esc(c.code)}">
      <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px;min-width:140px">
        <input type="checkbox" id="comp-enabled-${c.code}" ${checked ? 'checked' : ''} onchange="onSalaryComponentToggle('${c.code}')" style="width:auto">
        ${esc(c.name)}
      </label>
      <input type="number" id="comp-amount-${c.code}" placeholder="0" value="${amount}" style="width:130px" ${checked ? '' : 'disabled'}>
      <span style="font-size:11px;color:var(--text-muted)">₮/сар</span>
    </div>`;
  }).join('');
}

function onSalaryComponentToggle(code) {
  const checked = document.getElementById(`comp-enabled-${code}`).checked;
  const amountEl = document.getElementById(`comp-amount-${code}`);
  if (amountEl) { amountEl.disabled = !checked; if (!checked) amountEl.value = ''; }
}
...
async function readAndSaveEmployeeSalaryComponents(employeeDbId) {
  ...
  for (const c of catalog) {
    ...
    const enabled = enabledEl.checked;
    if (!enabled) {
      // Идэвхгvй бол мөрийг устгана (байвал)
      await sb.from('employee_salary_components').delete().eq('employee_id', employeeDbId).eq('component_code', c.code);
      continue;
    }
    const amount = +document.getElementById(`comp-amount-${c.code}`).value || 0;
    const row = { employee_id: employeeDbId, component_code: c.code, amount };
