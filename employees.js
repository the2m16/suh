...
function calculatePayrollGeneric(grossSalary, taxTypes, taxBrackets, employeeOverrides = []) {
  const breakdown = [];
  let nonProgressiveEmployeeDeductions = 0;
  let totalEmployerCost = grossSalary;
  ...
  for (const tt of ordered) {
    ...
    if (tt.calculation_type === 'simple') {
      ...
      const amt = +(grossSalary * rate / 100).toFixed(2);
      ...
      const empAmt = +(grossSalary * empRate / 100).toFixed(2);
      const erAmt = +(grossSalary * erRate / 100).toFixed(2);
      ...
      const base = Math.max(grossSalary - nonProgressiveEmployeeDeductions, 0);
  ...
  const netPay = +(grossSalary - totalEmployeeDeductions).toFixed(2);
  return { grossSalary, breakdown, netPay, totalEmployerCost: +totalEmployerCost.toFixed(2) };
}
...
async function runMonthlyPayroll() {
  ...
  for (const e of activeEmployees) {
    ...
    const r = calculatePayrollGeneric(e.baseSalary, config.taxTypes, config.brackets, overrides);
    const party = 'employee:' + e.dbId;
...
async function renderPayrollPreview() {
  ...
  if (headEl) {
    headEl.innerHTML = `<th>НЭР</th><th style="text-align:right">ҮНДСЭН ЦАЛИН</th>` +
  ...
  for (const e of activeEmployees) {
    const overrides = await loadEmployeeTaxOverrides(e.dbId);
    const r = calculatePayrollGeneric(e.baseSalary, taxTypes, brackets, overrides);
    ...
    rowsHtml.push(`<tr>
      ...
      <td class="dt-mono" style="text-align:right">${fmtMoney(r.grossSalary)}</td>
      ${taxCells}
