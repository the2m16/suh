  ...
  if (!list.length) {
    ...
    body.innerHTML = list.map((e, idx) => {
      ...
      <tr style="cursor:pointer" onclick="openEmployeeDetail(${e.id})">
        ...
        <td onclick="event.stopPropagation()">${_rowActionIcons(e.id, canEdit, canDel, 'editEmployee', 'deleteEmployee', canCopy('employees'), 'copyEmployee')}</td>
      </tr>`;
    }).join('');
  }
...
async function renderEmployeeSalaryComponentsUI(existingComponents = []) {
  ...
  container.innerHTML = catalog.map(c => {
    ...
    <div style="display:flex;align-items:center;gap:10px" data-comp-code="${esc(c.code)}">
      ...
      <span style="font-size:11px;color:var(--text-muted)">(${fmtMoney(c.amount)}₮/сар — бүх ажилтанд ижил, "НББ тохиргоо → Нэмэгдэл"-с тохируулна)</span>
    </div>`;
  }).join('');
}
...
async function editEmployee(id) {
  ...
  document.getElementById('employee-first-name').value = e.firstName || '';
...
// Хуулбарлах — editEmployee()-ийн бүх талбарыг (албан тушаал, цалин, банкны нэр гэх мэт
// загвар болгож болохуйц зүйлсийг) pre-fill хийгээд, editingEmployeeId-г null болгосноор
// saveEmployee() UPDATE биш шинэ INSERT хийнэ. Хувь хүний өвөрмөц талбарууд (нэр, регистр,
// ТТД, дансны дугаар) хоослогдож, хэрэглэгчээр шинээр оруулуулна.
async function copyEmployee(id) {
  if(!canCopy('employees')) { toast('Танд энэ үйлдлийг хийх эрх байхгүй байна','error'); return; }
  await editEmployee(id);
  editingEmployeeId = null;
  document.getElementById('modal-employee-title').textContent = 'Ажилтан хуулбарлах (нэр, регистр, ТТД-г шинээр оруулна уу)';
  document.getElementById('employee-name').value = '';
  document.getElementById('employee-register').value = '';
  document.getElementById('employee-ttd').value = '';
  document.getElementById('employee-bank-account').value = '';
...
async function saveEmployee() {
  ...
  const emp = {
    ...
    parentName: document.getElementById('employee-parent-name').value.trim(),
