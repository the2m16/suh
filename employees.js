...
async function renderPaySlipContent() {
  ...
  el.innerHTML = `
    ...
    <div class="summary-row"><span class="summary-key">Vндсэн цалин</span><span class="summary-val">${fmtMoney(baseSalary)}</span></div>
    ${allowanceRows}
    ...
    <div class="summary-row" style="font-weight:700"><span class="summary-key">ГАРТ ОЛГОХ ДvН</span><span class="summary-val">${fmtMoney(netPay)}</span></div>
    <div style="border-top:1px solid var(--border);margin:10px 0"></div>
...
// ============================================================
// ЦАЛИНГИЙН ТООЦООЛОЛ (Phase 2 -> ЕРӨНХИЙЛСӨН v2) -- цэвэр функцvvд,
// Node.js-д 21 тестээр баталгаажсан (payroll-calc-v2.js). Ямар ч тооны
// tax_types (applies_to_payroll=true) дэмждэг, ажилтан тус бvрийн
// employee_tax_overrides-ийг харгалзана. Шинэ татвар нэмэгдэхэд ЭНЭ
// КОД дахин бичигдэх шаардлагагvй.
// ⚠️ ЗӨВХӨН ТООЦООЛОЛ ХАРУУЛНА -- journal entry автоматаар vvсгэдэггvй.
// ============================================================
...
// цалингийн нэмэгдлvvд (Хоол/Унаа/Утас гэх мэт). Татвар бvр өөрийн
...
function calculatePayrollGeneric(baseSalary, taxTypes, taxBrackets, employeeOverrides = [], components = []) {
  ...
  for (const tt of ordered) {
    ...
    // Тухайн татварт "taxable=false" гэж тэмдэглээгvй бvх нэмэгдлийг суурьт оруулна
    // (анхдагч — талбар байхгvй бол taxable гэж vзнэ, хуулийн дагуу)
...
function buildPayrollLinesGeneric(party, result) {
  const lines = [];
  // Vндсэн цалин ТУСДАА (нэмэгдлvvдийг доор тус тусад нь бичнэ — Цалингийн
  // хуудсанд задалж харуулах боломжтой байхын тулд НЭГ 7010 руу нийлvvлдэггvй)
...
// ------------------------------------------------------------
// ЦАЛИНГИЙН НЭМЭГДЭЛ (Хоол/Унаа/Утас) — глобаль каталог (дvн, данс)
// + ажилтны хэрэглээ (зөвхөн идэвхтэй эсэх — дvн бvгдэд ижил тул
// ажилтан тус бvрт давхардуулж хадгалахгvй).
// ------------------------------------------------------------
...
async function ensureSalaryComponentsCache() {
  ...
  const { data } = await sb.from('salary_components').select('*').eq('enabled', true);
  _salaryComponentsCache = data || [];
  return _salaryComponentsCache;
}

// Ажилтны идэвхтэй нэмэгдлvvдийг calculatePayrollGeneric()-д шууд дамжуулах
// хэлбэрээр буцаана — дvн, ХХОАТ/НДШ тооцох эсэх, Дт данс бvгд ГЛОБАЛЬ
// каталогоос ирнэ (ажилтан бvрт ижил).
...
// ------------------------------------------------------------
// САР БvРИЙН ЦАЛИНГИЙН ЯВЦ (Phase 3) -- journal entry бодитоор vvсгэнэ
// ------------------------------------------------------------
...
async function renderPayrollPreview() {
  ...
  if (headEl) {
    headEl.innerHTML = `<th>НЭР</th><th style="text-align:right">НИЙТ ЦАЛИН</th>` +
...
function renderEmployeesTable(filter = '') {
  ...
  if (!list.length) {
    body.innerHTML = `<tr><td colspan="12" class="empty-state">Ажилтан олдсонгүй</td></tr>`;
  } else {
    body.innerHTML = list.map((e, idx) => {
      // Дансны дугаар: MN + IBAN 8 орон + банкны дансны дугаар (нэг мөрт нийлvvлж харуулна)
      ...
      <tr style="cursor:pointer" onclick="openEmployeeDetail(${e.id})">
        <td class="dt-muted dt-mono">${idx + 1}</td>
        <td class="dt-title">${esc(e.fullName)}</td>
...
// ------------------------------------------------------------
// АЖИЛТАН ТУС БvРИЙН ТАТВАРЫН OVERRIDE UI (Ажилтан нэмэх/засах modal)
...
async function ensureTaxTypesCache() {
  ...
  const { data } = await sb.from('tax_types').select('*').eq('applies_to_payroll', true).eq('enabled', true);
  ...
  container.innerHTML = taxTypes.map(t => {
    ...
    <div style="border:1px solid var(--border);border-radius:8px;padding:10px 12px" data-tax-code="${esc(t.code)}">
      ...
      <div id="tax-override-body-${t.code}" style="margin-top:8px;${enabled ? '' : 'display:none'}">
        ...
        ` : `<div style="font-size:11px;color:var(--text-muted)">Шаталсан тооцоолол хэвээр ажиллана — зөвхөн идэвхтэй/идэвхгvй сонголт боломжтой</div>`}
      </div>
...
// Modal доторх бvх татварын checkbox/талбарыг уншиж, Supabase руу upsert хийнэ.
...
// "Vндсэн цалин"-ы доор. Каталогийг динамикаар уншиж жагсаана.
// ------------------------------------------------------------
async function renderEmployeeSalaryComponentsUI(existingComponents = []) {
  ...
  container.innerHTML = catalog.map(c => {
    ...
    <div style="display:flex;align-items:center;gap:10px" data-comp-code="${esc(c.code)}">
      ...
      <span style="font-size:11px;color:var(--text-muted)">(${fmtMoney(c.amount)}₮/сар — бvх ажилтанд ижил, "НББ тохиргоо → Нэмэгдэл"-с тохируулна)</span>
    </div>`;
  }).join('');
}
...
async function readAndSaveEmployeeSalaryComponents(employeeDbId) {
  ...
  for (const c of catalog) {
    ...
    if (!enabledEl.checked) {
      await sb.from('employee_salary_components').delete().eq('employee_id', employeeDbId).eq('component_code', c.code);
      continue;
    }
...
async function saveEmployee() {
  ...
  if (ttd && !/^\d{12}$/.test(ttd)) { toast('ТТД яг 12 оронтой тоо байх ёстой', 'error'); return; }
