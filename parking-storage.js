...
function _populateSpotFilters(prefix, types) {
  ...
  bldSel.innerHTML = '<option value="">Бvх байр</option>'
    + bldIds.map(id=>{const b=BUILDINGS.find(x=>x.id===id); return `<option value="${id}">${esc(b?b.label:id+'-р байр')}</option>`}).join('')
    + (hasUnspecifiedBld ? '<option value="__none__">Тодорхойгvй (хамтын эзэмшил)</option>' : '');
  floorSel.innerHTML = '<option value="">Бvх давхар</option>' + floors.map(f=>`<option value="${esc(f)}">${esc(f)}</option>`).join('');
  zoneSel.innerHTML = '<option value="">Бvх бvсчлэл</option>' + zones.map(z=>`<option value="${esc(z)}">${esc(z)}-бүсчлэл</option>`).join('');
...
// нь хайна (Supabase-аас ачаалагдсан residents/businesses массиваас).
function _findSpotOwner(num, field) {
  const r = residents.find(x=>x && (x[field]||[]).includes(num));
  if(r) return {type:'resident', entity:r, name: ((r.firstname||'')+' '+(r.lastname||'')).trim()||'—'};
  const b = businesses.find(x=>x && (x[field]||[]).includes(num));
...
function renderParkingList() {
  ...
  PARKING_TYPES.forEach(t=>{
    ...
    nums.forEach(num=>{
      const owner = _findSpotOwner(num, 'parkings');
      rows.push({building: bld?bld.label:'Тодорхойгvй', floor:t.floor_label, zone:t.zone_label, num, sqm:t.sqm, owner});
    });
  });
  if(!rows.length){ body.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:20px;color:var(--text-muted)">Зогсоол бүртгэгдээгүй байна — эхлээд Хаягжилт тохиргоо хуудсанд нэмнэ vv</td></tr>'; return; }
...
function renderStorageList() {
  ...
  STORAGE_TYPES.forEach(t=>{
    ...
    nums.forEach(num=>{
      const owner = _findSpotOwner(num, 'storages');
      rows.push({building: bld?bld.label:'Тодорхойгvй', floor:t.floor_label, zone:t.zone_label, num, sqm:t.sqm, owner});
    });
  });
  if(!rows.length){ body.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:20px;color:var(--text-muted)">Агуулах бүртгэгдээгүй байна — эхлээд Хаягжилт тохиргоо хуудсанд нэмнэ vv</td></tr>'; return; }
  body.innerHTML = rows.map(row=>`<tr style="cursor:${row.owner?'pointer':'default'}" ${row.owner?`onclick="openSpotOwnerEdit('${row.owner.type}',${row.owner.entity.id})"`:''}>
    <td class="dt-text">${esc(row.building)}</td>
    <td class="dt-muted">${esc(row.floor)||'—'}</td>
    <td class="dt-muted">${esc(row.zone)||'—'}</td>
